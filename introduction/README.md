...
|No.|内容|更新日|
|:--|:--|:--:|
|001|[星型](#014_001)|2020-06-04|
|002|[ベベル･面取り](#014_002)|2020-06-04|
|003|[蛇腹](#014_003)|2020-06-04|
|004|[バネ](#014_004)|2020-06-04|
|005|[壁にパイプを結合](#014_005)|2020-06-04|
|006|[歯車](#014_006)|2020-06-04|
|007|[ネジ山](#014_007)|2020-06-04|
|008|[ハート型に掘る](#014_008)|2020-06-04|
...
1. 左側のアイコン群の中から [拡大縮小]（上から5番目）を選択
...
1. [オブジェクト]-[ペアレント]-[オブジェクト] を選択し次の通りに設定
    * タイプ：オブジェクト
    * トランスフォーム維持：✔
1. [シーンコレクション] 内で "円" が "エンプティ_バネ用" にネスト化されたのを確認
...
#### ◆穴をあける
1. [ビュー]-[視点]-[前] に変更
1. 左側のアイコン群の中から [ループカット] を選択（上から12番目）
1. ループカットしたい面でとりあえず左クリック
...
1. 全てを選択（Aキー）した状態で [頂点]-[頂点からの新規辺/面]
...
1. 上記の"歯車のみを選択"した状態でスパナアイコン
    ![014_004](https://mubirou.github.io/Blender/introduction/jpg/014_004_1.jpg)
    -[モディファイアーを追加]-[ブーリアン] を選択し各種設定し [適用] をクリック
    * 演算：差分
    * オブジェクト：円柱 ←上記で作成したもの
...
1. スパナアイコン![014_004](https://mubirou.github.io/Blender/introduction/jpg/014_004_1.jpg)
    -[モディファイアーを追加]-[ブーリアン]を選択し各種設定し [適用]
    * 演算：差分
...
1. [選択]-[なし] で選択を全て解除
1. 左側のアイコン群から [ナイフ] アイコン（上から13番目）を選択
...
#### ◆下絵の読込み
1. Blender で 3D ビューを3つ（①フロント･並行投影 ②ライト･並行投影 ③ユーザー･並行投影）用意する
1. "①フロント･並行投影" の3Dビューを選択
1. デスクトップ上の画像ファイルを"①フロント･並行投影"の領域にドラッグ＆ドロップ
1. 画面右側のアイコン群に登場したオブジェクトデータプロパティ
![014_009_4](https://mubirou.github.io/Blender/introduction/jpg/014_009_4.jpg)
タブを選択し調整  
    * サイズ：任意（初期値5m）←モデリングサイズはここで確定しておく
    * サイド：両方（初期値）
    * 透過：**✔**
    * 不透明度：**0.2**（0.000〜1.000）
1. "①フロント･並行投影" の 3D ビューを選択
1. [シーンコレクション] 上で上記の下絵を複製
1. 3D ビューを "上" にして90度回転、上記同様アルファ値を **0.2** に変更
1. [移動] を使って微調整（正面と横からのイラスト部分が重なるようにする）  
...
1. 何れかの3Dビュー画面左上アイコンから[エディター]タイプを[UV画像エディター]に変更
1. テクスチャを貼りたい面を選択
...
1. 何れかの画面の[エディター]タイプを[UVエディター]に変更 ←要注意
1. [UVエディター]画面の[開く]ボタンから、上記のサイコロ展開図（.pngファイル）を選択
1. パスの全てを選択（[A]キー）
1. [UVエディター]画面上で[T]キー→[Transform]アイコン（上から3つ目）を選択
1. ポイント･ハンドルを選択して移動･回転するなどして（コンテキストメニューにも機能有）、テクスチャと展開図を合わせる  
    ![015_005_4](https://mubirou.github.io/Blender/introduction/jpg/015_005_4.jpg) 
1. [UVエディター]画面上部の[画像]メニュー→[パック]を選択
1. 何れかの画面の[エディター]タイプを[シェーダーエディター]に変更  
    （この状態では空）
1. [編集モード]の画面で、テクスチャを貼りたい面を選択している事を確認  
    （常に[ルック開発モード]か[レンダープレビュー]で表示します） ←要注意!
1. ![015_004_5](https://mubirou.github.io/Blender/introduction/jpg/015_004_5.jpg)
    アイコンを押し→[新規]ボタンを選択
1. マテリアル名を、"サイコロ用マテリアル"にする（任意）  
    （[シェーダーエディター]上にノードが追加される）
1. [シェーダーエディター]画面で[追加]-[テクスチャ]-[画像テクスチャ]でノードを追加
1. 既存の[プリンシブルBSDF]ノードの左側に[画像テクスチャ]ノードを配置
1. [画像テクスチャ]の[カラー]↔[プリンシブルBSDF]の[ベースカラー]を接続
1. [画像テクスチャ]ノードの[開く]から、上記のサイコロ展開図（.png）を選択
1. [画像テクスチャ]（.png）ノードの設定を[ボックス]→[フラット]に変更 ←要注意!
1. 引き続きさらに次の2つのノードを追加  
    1. [追加]-[ベクトル]-[マッピング]
    1. [追加]-[入力]-[テクスチャ座標]
1. 上記を次の通りに接続
    1. [マッピング]の[ベクトル]↔[画像テクスチャ]（.png）の[ベクトル]
    1. [マッピング]の[ベクトル]↔[テクスチャ座標]の[UV]  
