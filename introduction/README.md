...
1. [追加]-[画像]-[参照] から上記と同じ下絵を選択
1. 3D ビューを "上" にして90度回転、上記同様アルファ値を **0.2** に変更
1. [Move] を使って微調整（正面と横からのイラスト部分が重なるようにする）
...
1. [追加]-[メッシュ]-[円]や[平面] などを次の機能などを使って編集をしてパイプとなる[頂点]と[辺]で構成されるオブジェクトを以下の機能を使うなどして作成  
    ...
    * [オブジェクト]-[変換]-[カーブ/メタ/サーフェス/テキストからメッシュ] → [編集モード] → [Knife]  
...
1. 3D ビューの右の [<] から [トランスフォーム] を表示して調整
    * 辺データ：平均クリース：1.00  
...
1. [作成]-[立方体]で立方体を作成
1. 左側のアイコン群の中から[LoopCut]で窓の位置に合わせて分割
